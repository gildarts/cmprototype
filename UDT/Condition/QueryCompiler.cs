                    ...
                    while (_Symbols.Count > ++i)
                    {
                        if (_Symbols[i].Value == ",")
                        {
                            if (_Symbols.Count <= i + 1 || _Symbols[i - 1].Thing != "Value" || _Symbols[i + 1].Thing != "Value")
                                throw new CompilerFailedException(query, "002", _Symbols[i], "\" ,\"必需置於兩個值中間 ");
                        }
                        ...
                        else if (_Symbols[i].Value == ")")
                        {
                            ...
                            throw new CompilerFailedException(query, "003", _Symbols[i], "不允許的內容 ");
                    }
                    if (!finish || !hasValue)
                        throw new CompilerFailedException(query, "004", item,"沒有( 或沒有值。");
                }
                if (item.Thing == "Value" && !(item.Value == "null" || ValidateType(DataType.String, item.Value) || ValidateType(DataType.Boolean, item.Value) || ValidateType(DataType.DateTime, item.Value) || ValidateType(DataType.Number, item.Value)))
